...
一条消息变成一个页面。第一句话作为标题，状态设成你状态列的第一个选项，分类
留空，正文按三段写好，方便你以后回来慢慢想：
...
建一个数据库，有一个标题属性；想要的话再加一个 **单选** 属性当状态、一个
**多选** 属性当分类。名字随便取，中英文都行。bot 读你的数据库结构、按**类型**
认列，不认名字——所以中文的 `想法 / 状态 / 分类` 和英文的 `Idea / Status /
Category` 都能零配置直接用，事后改列名也不会坏。
...
正常你**不用配任何字段名**——bot 按类型自动认列。新页面的状态取你状态列的
**第一个选项**（你叫它什么、什么语言都行），分类留空，等你回顾时自己标。消息
里带链接、而你库里正好有个 URL 列，链接也会自动填进去。

覆盖变量只为一种情况准备：库里有**两个同类型的列**，自动认不出哪个是哪个（比如
一个 `状态` 单选紧挨着一个 `优先级` 单选），这时手动指定正确的那个。
[`.env.example`](.env.example) 里每个变量都可以当 secret 用：

| Secret | 默认 | 作用 |
|---|---|---|
| `IDEA_TITLE_PROPERTY` | 自动（title 列） | 指定标题列 |
| `IDEA_STATUS_PROPERTY` | 自动（单选列） | 指定状态列 |
| `IDEA_CATEGORY_PROPERTY` | 自动（多选列） | 指定分类列 |
| `IDEA_URL_PROPERTY` | 自动（url 列） | 指定链接列 |
| `IDEA_DEFAULT_STATUS` | 状态列第一个选项 | 新页面的状态 |
| `IDEA_DEFAULT_CATEGORY` | 留空 | 新页面的分类 |
